...
class BaseService:
  def __init__(self, repo:BaseRepository, db_session: AsyncSession):
    self.base_repo = repo
    self.session = db_session

async def get_object_by_id(self, object_id:int):
    current_object = await self.base_repo.get_by_id(self.session, object_id)
...
async def get_object_by_params(self, **filters)->object|None:
        ...
        if not filters:
            ...
            return False
        current_object = await self.base_repo.get_by_params(self.session, **filters)
        if current_object is None:
...
async def get_objectS_by_params(self, **params):
        if not params:
            return None
        current_objects = await self.base_repo.get_objects_by_params(self.session, **params)
...
async def get_all_objects(self, skip: int = 0, limit: int = 100) -> Sequence[object]:  
    all_objects = await self.base_repo.get_all(skip=skip, limit=limit)
    return all_objects

async def create_object(self, new_object_data:dict) -> object | None:
    existing_object = await self.base_repo.get_by_params(new_object_data)
    if existing_object:
      raise ValueError(f"объекта с параметрами : {new_object_data}  уже существует")
    new_object = await self.base_repo.create(self.session, new_object_data)
...
async def update_put(self, session: AsyncSession,  object_id:int ,update_data: dict[str, Any]):
    ...
    updated_result = await self.base_repo(self.session, object_id, update_data)
